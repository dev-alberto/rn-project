...
class Memory:
    def __init__(self, size=1000000, screen_height=84, screen_width=84, history_length=4, batch_size=32, min_reward=-1,
                 max_reward=1):
        self.size = size
        # preallocate memory
        self.actions = np.empty(self.size, dtype=np.uint8)
        self.rewards = np.empty(self.size, dtype=np.int64)
